...
namespace HARMOLOID_Csharp
{
    public class DATA
    {
                ...
                case 3: // load ust for UTAU
                    {
                        ...
                        if (!USTValid)
                        {
                            MessageBox.Show("文件损坏或不包含任何音符，无法载入文件。", "载入");
        ...
        public void DATASave(string SavingPath, int HarmoNum) // save ust for UTAU
        {
            ...
            {
                MessageBox.Show("保存UST时，请勿覆盖原文件。", "保存");
                return;
            }
    ...
    public class TRACK
    {
        ...
        public bool AutoTonalize1()
        {
            ...
            for (int PassageNum = 0; PassageNum < PassageNumTotal; PassageNum++)
            {
                ...
                if (PassageList[PassageNum].IsSure())
                {
                    for (int i = 0; i < 12; i++)
                    {
                        if (PassageList[PassageNum].TonalityOption[i] == 3)
                        {
                            ...
                            OptionsGiven += "Passage" + PassageNum + "（小节" + PassageList[PassageNum].FirstBarNum + "~小节" + PassageList[PassageNum].LastBarNum + "）分析成功，调性为" + Constants.Tonality_Type[PassageList[PassageNum].Tonality] + "。\r\n";
                        }
                    }
                }
                ...
                {
                    AllTonalized = false;
                    OptionsGiven += "Passage" + PassageNum + "（小节" + PassageList[PassageNum].FirstBarNum + "~小节" + PassageList[PassageNum].LastBarNum + "）分析失败，请自行填写调性，或者选择“无调”忽略该乐段。\r\n";
                }
                else
                {
                    AllTonalized = false;
                    OptionsGiven += "Passage" + PassageNum + "（小节" + PassageList[PassageNum].FirstBarNum + "~小节" + PassageList[PassageNum].LastBarNum + "）分析失败，建议您从" + PassageList[PassageNum].GetTonalityOptionAsString() + "中选择调性。\r\n";
                }
            }
            MessageBox.Show(OptionsGiven.Remove(OptionsGiven.Length - 2), "调性计算结果");
            return AllTonalized;
        }
        public bool AutoTonalize2()
        {
            ...
            for (int i = 0; i < BarNumTotal; i++)
            {
                ...
                {
                    MessageBox.Show("音轨过短，无法进行自动计算", "错误");
                    return false;
                }
                pBarNum += 3;
